      ...
      <div className="container mx-auto px-6">
        ...
        >
          ...
          <h3 className="text-2xl font-bold mb-8 flex items-center gap-2">
            My Arsenal <span className="text-slate-500 text-sm font-normal">/ Technologies I use</span>
          </h3>
          ...
          <div className="flex flex-wrap gap-3">
            {skills.map((skill, idx) => (
              <span 
                key={idx}
                className="px-4 py-2 rounded-full bg-white/5 border border-white/10 text-slate-300 hover:bg-white/10 hover:text-white transition-colors cursor-default"
              >
                {skill.name}
              </span>
            ))}
